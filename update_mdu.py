...
def load_cache():
    if os.path.exists(cache_path):
        try:
            ...
            print(f"Erro ao carregar cache geogrÃ¡fico: {e}")
    return {}
...
def save_cache(cache):
    try:
        ...
        print(f"Erro ao salvar cache geogrÃ¡fico: {e}")
...
def clean_and_normalize(addr):
    ...
    # Remove block/apt/etc info at the end (e.g. "BL 1", "APTO 203", "ÃšNICO", "UNICO", "TORRE K", etc)
    addr = re.sub(r'\s*-\s*(ÃšNICO|UNICO|COMPLETO)$', '', addr)
    ...
    # CorreÃ§Ãµes de ortografia de ruas comuns para aumentar a taxa de geocodificaÃ§Ã£o
    ...
    addr = addr.replace("LEÃƒO XIII", "LEAO 13")
...
def get_address_variations(raw_addr):
    ...
    expanded = re.sub(r'^(PC|PRC)\b\.?\s*', 'PRAÃ‡A ', expanded, flags=re.IGNORECASE)
    ...
    prefixless = re.sub(r'^(RUA|AVENIDA|TRAVESSA|ESTRADA|PRAÃ‡A|ACESSO)\s+', '', corrected, flags=re.IGNORECASE)
    if prefixless != corrected:
        ...
        raw_name = re.sub(r'^(DESEMBARGADOR|DOUTOR|DOUTORA|PROFESSOR|BARÃƒO|VISCONDE|CONDE|CORONEL|GENERAL|REVERENDO|PADRE|PRESIDENTE|GOVERNADOR|MINISTRO|SENADOR|PREFEITO)\s+', '', prefixless, flags=re.IGNORECASE)
...
def get_uf(cidade):
    # Todos do CSV atual estÃ£o na regiÃ£o metropolitana de Porto Alegre (RS)
    return "RS"
    ...
    try:
        with urllib.request.urlopen(req, timeout=10) as response:
            ...
            if res_data:
                ...
                print(f"  EndereÃ§o nÃ£o localizado pelo Nominatim: {address_str}")
    except Exception as e:
