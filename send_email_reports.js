...
// Utilitário para requisições HTTPS nativas (Node.js 18+)
...
function getMduStatusCounts() {
    const jsPath = path.join(__dirname, 'mdu_data.js');
    if (!fs.existsSync(jsPath)) {
        console.warn(`Arquivo mdu_data.js não encontrado em: ${jsPath}`);
        return null;
    }
    ...
    if (!dataMatch) {
        console.warn("Não foi possível localizar window.MDU_DATA no arquivo.");
        return null;
    }
...
function buildEmailHtml(data, reportName) {
    ...
    statusOrder.forEach(s => {
        // Encontrar chave correspondente ignorando acentos/maiusculas
        let cnt = 0;
    ...
    Object.keys(data.counts).forEach(k => {
        const isMapped = statusOrder.some(s => 
            s.key.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") === 
...
async function start() {
    try {
        console.log("Buscando configurações de e-mail no Supabase...");
        ...
        if (!mduData) {
            console.error("Não foi possível computar os dados do MDU.");
            return;
        }
