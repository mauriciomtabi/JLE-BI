...
def process_csv():
    ...
    print("Processando dados de Manutenção a partir do Google Sheets...")
    rows = []
    with open(csv_path, mode='r', encoding='utf-8-sig') as f:
        ...
        for r in reader:
            ...
            # Considerar APENAS linhas com Status válido na Coluna I do Google Sheets
            if not status or status.upper() in ('', '-', 'STATUS'):
                continue
            
            rows.append(r)

    print(f"Total de registros operacionais válidos (do Google Sheets): {len(rows)}")
    ...
    print(f"Cruzamento concluído: {matched_fin_count} OFs com valor medido > 0. Valor Total Medido: R$ {total_fin_value:,.2f}")
